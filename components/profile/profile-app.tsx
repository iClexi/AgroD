...
function ProfileFact({ icon: Icon, label, value }: { icon: typeof MapPin; label: string; value: string }) { return <div><dt className="flex items-center gap-3 text-xs text-white/60"><Icon aria-hidden="true" className="size-4 shrink-0 text-[#8ed09f]" />{label}</dt><dd className="ml-7 mt-1 text-sm font-semibold text-white/85">{value}</dd></div> }
