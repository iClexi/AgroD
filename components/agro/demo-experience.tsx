      ...
      <div className="mx-auto max-w-6xl px-5 md:px-8">
        <Reveal className="overflow-hidden rounded-[2rem] bg-navy text-navy-foreground shadow-[0_40px_100px_-55px_rgba(7,31,66,.85)]">
          <div className="grid lg:grid-cols-[.72fr_1.28fr]">
            <div className="relative overflow-hidden px-6 py-12 md:px-10 lg:px-12 lg:py-16">
              <div aria-hidden="true" className="demo-field-bg absolute inset-0 opacity-25" />
              <div className="relative"><p className="text-sm font-semibold uppercase tracking-wider text-[#8ed09f]">Pruébalo antes de crear tu cuenta</p><h2 className="mt-3 text-balance font-display text-4xl font-bold leading-tight md:text-5xl">Recorre AgroD con una finca de ejemplo.</h2><p className="mt-5 text-lg leading-relaxed text-white/72">No necesitas instalar nada. Cambia entre las vistas, revisa prioridades y prueba cómo localizarías un equipo antes de empezar con tus propios cultivos.</p>
                <ul className="mt-8 space-y-3 text-sm text-white/78"><li className="flex gap-3"><CheckCircle2 aria-hidden="true" className="mt-0.5 size-5 shrink-0 text-[#8ed09f]" />Prioridades claras para la inspección</li><li className="flex gap-3"><MapPinned aria-hidden="true" className="mt-0.5 size-5 shrink-0 text-[#8ed09f]" />Cultivos ubicados dentro de la finca</li><li className="flex gap-3"><Radio aria-hidden="true" className="mt-0.5 size-5 shrink-0 text-[#8ed09f]" />Equipos asociados a cada zona</li></ul>
                <div className="mt-10 flex flex-col gap-3 sm:flex-row"><Link href="/registro" className="group inline-flex h-12 items-center justify-center gap-2 rounded-full bg-primary px-7 text-sm font-semibold text-primary-foreground transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">Crear mi finca en AgroD<ArrowRight aria-hidden="true" className="size-4 transition-transform group-hover:translate-x-0.5" /></Link><Link href="/iniciar-sesion" className="inline-flex h-12 items-center justify-center rounded-full border border-white/25 px-7 text-sm font-semibold text-white transition-colors hover:bg-white/10">Ya tengo cuenta</Link></div>
              </div>
            </div>

            <div className="bg-[#f4f8f4] p-3 text-foreground sm:p-5 lg:p-7">
              <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-xl">
                ...
                <nav className="grid grid-cols-2 border-b border-border/70 bg-background/70 sm:grid-cols-4" aria-label="Vistas de la demostración">{views.map((item) => <button type="button" key={item.id} onClick={() => setView(item.id)} className={`flex min-h-14 items-center justify-center gap-2 px-2 text-xs font-semibold transition-colors ${view === item.id ? 'bg-card text-primary shadow-[inset_0_-2px_0_var(--primary)]' : 'text-muted-foreground hover:bg-card hover:text-foreground'}`}><item.icon aria-hidden="true" className="size-4" />{item.label}</button>)}</nav>
                <div key={view} className="demo-panel-in min-h-[430px] p-4 sm:p-6">
...
function SummaryDemo() {
  return <><div><p className="text-xs font-semibold text-primary">La Vega · Lecturas ilustrativas</p><h3 className="mt-1 font-display text-2xl font-bold">Buenos días. Estas son tus prioridades.</h3></div><div className="mt-5 grid grid-cols-2 gap-3 xl:grid-cols-4"><DemoMetric icon={Droplets} label="Humedad" value="41%" /><DemoMetric icon={ThermometerSun} label="Temperatura" value="29 °C" /><DemoMetric icon={AlertTriangle} label="Atención" value="2 zonas" /><DemoMetric icon={Signal} label="Conectados" value="2 equipos" /></div><div className="mt-5 grid gap-4 sm:grid-cols-[1.15fr_.85fr]"><div className="rounded-2xl border border-border p-4"><div className="flex items-center justify-between"><p className="font-semibold">Finca virtual</p><span className="text-xs font-semibold text-primary">Ruta activa</span></div><div className="mini-farm mt-3 h-[185px]"><span className="mini-marker marker-one">1</span><span className="mini-marker marker-two">2</span><span className="mini-marker marker-three">3</span></div></div><div className="rounded-2xl bg-navy p-4 text-navy-foreground"><div className="flex items-center gap-2"><Route aria-hidden="true" className="size-4 text-[#8ed09f]" /><p className="font-semibold">Orden recomendado</p></div><ol className="mt-4 space-y-3 text-sm text-white/75"><li>1. Ají Sur</li><li>2. Tomate Norte</li><li>3. Plátano Este</li></ol></div></div></>
}
