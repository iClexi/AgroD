...
const benefits = [
  { icon: Siren, feature: 'Prioridad por riesgo', title: 'Empieza por lo urgente', text: 'AgroD ordena las zonas que requieren atención para que no pierdas tiempo revisando todo por igual.' },
  { icon: MapPinned, feature: 'Mapa, ruta y zumbador', title: 'Encuentra la zona correcta', text: 'Ubica cada cultivo, sigue un orden de inspección y haz sonar el dispositivo compatible cuando lo necesites.' },
  { icon: Activity, feature: 'Lecturas e historial', title: 'Decide con contexto', text: 'Compara humedad, temperatura, alertas y acciones anteriores antes de intervenir el cultivo.' },
]
      ...
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        ...
        <div className="mt-9 grid gap-4 lg:grid-cols-3">
          {benefits.map((benefit, index) => (
            <Reveal key={benefit.title} delay={index * 90} className="group rounded-2xl border border-border/70 bg-card p-6 transition-all duration-300 hover:-translate-y-1 hover:border-primary/40 hover:shadow-lg">
              <div className="flex items-center justify-between gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary transition-colors duration-300 group-hover:bg-primary group-hover:text-primary-foreground"><benefit.icon aria-hidden="true" className="h-5 w-5" /></span><ArrowRight aria-hidden="true" className="size-4 text-border transition-transform group-hover:translate-x-1 group-hover:text-primary" /></div>
